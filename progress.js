function allExerciseKeys(){
  const map=new Map();
  for(const ex of state.sessionExercises){const p=safeJson(ex.payload);if(!map.has(ex.exercise_key))map.set(ex.exercise_key,{key:ex.exercise_key,name:p.englishName||p.name||ex.exercise_key,sk:p.name||''});}
  for(const ex of state.planExercises){const p=safeJson(ex.payload);if(!map.has(ex.exercise_key))map.set(ex.exercise_key,{key:ex.exercise_key,name:p.englishName||p.name||ex.exercise_key,sk:p.name||''});}
...
function renderExerciseDetail(idOrKey){
  ...
  const content=`<div class="detail-head"><button class="back-btn" id="back">${icon('back')}</button><div><div class="eyebrow">${h(ep.name||'Cvik')}</div><h1>${h(ep.englishName||ep.name||key)}</h1><p>${h(muscles.join(' · '))}</p></div></div><div class="prescription" style="padding:0 0 10px"><span class="chip">${h(ep.sets??'—')}×${h(ep.repMin??'?')}${ep.repMax&&ep.repMax!==ep.repMin?`–${h(ep.repMax)}`:''}</span><span class="chip">RIR ${h(ep.rirMin??'—')}${ep.rirMax&&ep.rirMax!==ep.rirMin?`–${h(ep.rirMax)}`:''}</span><span class="chip accent">${fmtNumber(ep.weight)} kg</span><span class="chip">${h(restLabel(ep))}</span></div>
